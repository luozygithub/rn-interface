		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('VoteId', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
...
incrVoteId ({rootState} ){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.incrVoteId().estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.incrVoteId().send({
                                from: rootState.app.account,
