		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('RbContribution', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
_RBD ({rootState} ){
					...
					return new Promise((resolve,reject) => {
						state.token.methods._RBD().call().then(res => {
						resolve(res)
					}).catch(err=>{
...
bankAddress ({rootState} ){
					judgeToken(rootState)
					return new Promise((resolve,reject) => {
						state.token.methods.bankAddress().call().then(res => {
						resolve(res)
					}).catch(err=>{
                        reject(JSON.parse(err.message.substr(24,err.message.length)).message)
                    })
...
setrbConsensus ({rootState}, addr){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.setrbConsensus(addr).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.setrbConsensus(addr).send({
                                from: rootState.app.account,
                                gas: parseInt(gas * 1.2)
                            }).then(res => {
                                 let operateLogs = localStorage.getItem("operateLogs")?JSON.parse(localStorage.getItem("operateLogs")):[]
