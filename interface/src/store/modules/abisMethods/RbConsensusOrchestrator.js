		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('RbConsensusOrchestrator', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
...
rbConsersusAddress ({rootState} ){
					judgeToken(rootState)
					return new Promise((resolve,reject) => {
						state.token.methods.rbConsersusAddress().call().then(res => {
						resolve(res)
					}).catch(err=>{
                        reject(JSON.parse(err.message.substr(24,err.message.length)).message)
                    })
...
init ({rootState}, {RBT,VIP}){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.init(RBT,VIP).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.init(RBT,VIP).send({
                                from: rootState.app.account,
                                gas: parseInt(gas * 1.2)
                            }).then(res => {
                                 let operateLogs = localStorage.getItem("operateLogs")?JSON.parse(localStorage.getItem("operateLogs")):[]
