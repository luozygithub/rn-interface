		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('RbSeedSubscriptionOrchestrator', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
RbSeedSubscriptionAddress ({rootState} ){
					judgeToken(rootState)
					return new Promise((resolve,reject) => {
						state.token.methods.RbSeedSubscriptionAddress().call().then(res => {
						resolve(res)
					}).catch(err=>{
...
init ({rootState}, seed){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.init(seed).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.init(seed).send({
                                from: rootState.app.account,
                                gas: parseInt(gas * 1.2)
                            }).then(res => {
                                 let operateLogs = localStorage.getItem("operateLogs")?JSON.parse(localStorage.getItem("operateLogs")):[]
