		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('RbtDepositOrchestrator', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
...
input ({rootState}, _bank){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.input(_bank).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.input(_bank).send({
                                from: rootState.app.account,
                                gas: parseInt(gas * 1.2)
                            }).then(res => {
                                 let operateLogs = localStorage.getItem("operateLogs")?JSON.parse(localStorage.getItem("operateLogs")):[]
