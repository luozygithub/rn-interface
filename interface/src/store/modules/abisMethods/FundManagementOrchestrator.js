		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('FundManagementOrchestrator', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
FundManagementAddr ({rootState} ){
					judgeToken(rootState)
					return new Promise((resolve,reject) => {
						state.token.methods.FundManagementAddr().call().then(res => {
						resolve(res)
					}).catch(err=>{
