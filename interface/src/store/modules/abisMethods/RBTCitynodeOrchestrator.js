		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('RBTCitynodeOrchestrator', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
...
init ({rootState}, {core,_exchangeGovernance721}){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.init(core,_exchangeGovernance721).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.init(core,_exchangeGovernance721).send({
                                from: rootState.app.account,
                                gas: parseInt(gas * 1.2)
                            }).then(res => {
                                 let operateLogs = localStorage.getItem("operateLogs")?JSON.parse(localStorage.getItem("operateLogs")):[]
