		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('RbtVipOrchestrator', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
...
init ({rootState}, {price,foundationAddress,rbtex,core}){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.init(price,foundationAddress,rbtex,core).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.init(price,foundationAddress,rbtex,core).send({
                                from: rootState.app.account,
                                gas: parseInt(gas * 1.2)
                            }).then(res => {
                                 let operateLogs = localStorage.getItem("operateLogs")?JSON.parse(localStorage.getItem("operateLogs")):[]
