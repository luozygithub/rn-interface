		...
		function judgeToken(rootState) {
			if (!state.token) state.token = getContract.getContractByName('RbinviteOrchestrator', rootState.app.web3)
		}
		const state = {};
		const mutations = {};
	 const actions = { 
...
rbinviteAddress ({rootState} ){
					judgeToken(rootState)
					return new Promise((resolve,reject) => {
						state.token.methods.rbinviteAddress().call().then(res => {
						resolve(res)
					}).catch(err=>{
                        reject(JSON.parse(err.message.substr(24,err.message.length)).message)
                    })
...
init ({rootState}, {RBTEX,RBT,RBCONSENSUS}){
                        judgeToken(rootState)
                        return new Promise((resolve,reject) => {
                            state.token.methods.init(RBTEX,RBT,RBCONSENSUS).estimateGas({
                            from: rootState.app.account,
                        }).then(gas => {
                            state.token.methods.init(RBTEX,RBT,RBCONSENSUS).send({
                                from: rootState.app.account,
                                gas: parseInt(gas * 1.2)
                            }).then(res => {
                                 let operateLogs = localStorage.getItem("operateLogs")?JSON.parse(localStorage.getItem("operateLogs")):[]
